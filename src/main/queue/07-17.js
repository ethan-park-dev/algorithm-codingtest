/**
 * 문제 17 카드 뭉치
 * 문제 URL: https://programmers.co.kr/learn/courses/30/lessons/159994
 * 정답 URL: https://github.com/kciter/coding-interview-js/blob/main/solution/17.js
 *
 * 문제 설명:
 *  코니는 영어 단어가 적힌 카드 뭉치 2개를 선물 받았습니다. 코니는 다음과 같은 규칙으로 카드에 적힌 단어들을 사용해
 *  원한느 순서의 단어 배열을 만들 수 있는지 알고 싶습니다.
...
export default function solution(cards1, cards2, goal) {
  let idx1 = 0; // cards1의 현재 위치
  let idx2 = 0; // cards2의 현재 위치

  for (let word of goal) {
    // cards1에서 단어를 찾을 수 있는지 확인
    if (idx1 < cards1.length && cards1[idx1] === word) {
      idx1++; // 찾았으면 cards1의 포인터를 이동
    }
    // cards2에서 단어를 찾을 수 있는지 확인
    else if (idx2 < cards2.length && cards2[idx2] === word) {
      idx2++; // 찾았으면 cards2의 포인터를 이동
    }
    // 어느 쪽에서도 현재 단어를 찾을 수 없는 경우
    else {
      return 'No';
    }
  }
