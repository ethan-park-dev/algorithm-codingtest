 ...
 *
 * 문제 설명
 *  당신은 폰켓몬을 잡기 위한 오랜 여행 끝에, 홍 박사님의 연구실에 도착했습니다. 홍 박사님은 당신에게
 *  자신의 연구실에 있는 총 N 마리의 폰켓몬 중에서 N/2마리를 가져가도 좋다고 했습니다.홍 박사님 연구실의 폰켓몬은
 *  종류에 따라 번호를 붙여 구분합니다. 따라서 같은 종류의 폰켓몬은 같은 번호를 가지고 있습니다.
...
export default function solution(nums) {
  const uniqueTypes = new Set(nums); // 중복 제거하여 폰켓몬 종류 구하기
  const maxSelectable = nums.length / 2; // N/2 마리 선택 가능

  return Math.min(uniqueTypes.size, maxSelectable); // 종류 수와 N/2 중 작은 값 반환
}
